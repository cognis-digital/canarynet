...
TOOL_NAME = "CANARYNET"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'CN-HIT-001', 'severity': 'critical', 'weight': 3.0, 'title': 'CANARY_TRIGGERED', 'when': {'triggered': True}, 'remediation': 'Treat as confirmed intrusion. Begin IR immediately.'}, {'id': 'CN-EXP-001', 'severity': 'low', 'weight': 1.5, 'title': 'CANARY_EXPIRED', 'when': {'expired': True, 'triggered': False}, 'remediation': 'Rotate and redeploy canary.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="canary",
                    metadata=item,
                ))
