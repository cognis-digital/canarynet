...
How **canarynet** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `canarynet` lives in the **Network / recon** cluster.
...
| from | relation | to |
|---|---|---|
| `canarynet` | JSON findings compose with | [`nmapparse`](https://github.com/cognis-digital/nmapparse), [`pcapview`](https://github.com/cognis-digital/pcapview), [`dnsrecon`](https://github.com/cognis-digital/dnsrecon), [`subhunt`](https://github.com/cognis-digital/subhunt) |
| `canarynet` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `canarynet` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `canarynet` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the network / recon cluster.** `canarynet` output feeds the next tool:
```bash
canarynet ... --format json > out.json          # this tool's findings (see `canarynet --help`)
...
canarynet ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
canarynet ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
canarynet ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `canarynet`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `canarynet` slots
